...
static esp_err_t esp_io_expander_new_i2c_tca9554(i2c_port_t i2c_num, uint32_t i2c_address, esp_io_expander_handle_t *handle);

void ESP_IOExpander_TCA9554::begin(void)
{
    CHECK_ERROR_RETURN(esp_io_expander_new_i2c_tca9554(i2c_id, i2c_address, &handle));
}
...
static esp_err_t read_input_reg(esp_io_expander_handle_t handle, uint32_t *value)
{
    ...
    // *INDENT-OFF*
    ESP_RETURN_ON_ERROR(
        i2c_master_write_read_device(tca9554->i2c_num, tca9554->i2c_address, &reg, 1, &temp, 1, pdMS_TO_TICKS(I2C_TIMEOUT_MS)),
